...
object RecipientTableGlue {
   ...
   * @See TI_IdentityTable.getCursorForTIUnlocked()
   */
  @Suppress("KDocUnresolvedReference") // just cause TI_IdentityTable is now internal
  @JvmStatic
  @SuppressLint("Range")
  fun getValidTICandidates(cursorForTIUnlocked: Cursor): Map<RecipientId, RecipientRecord> {
  ...
  private fun getRecipientIdsFromACIs(cursor: Cursor? = null, acis: Collection<String>? = null): Map<RecipientId, RecipientRecord> {
    val serviceIdentifiers = arrayListOf<String>()
      ...
      if (cursor.moveToFirst()) {
        cursor.use {
          while (!it.isAfterLast) {
            serviceIdentifiers.add(it.getString(it.getColumnIndex(IdentityTable.ADDRESS)))
            it.moveToNext()
          }
        }
      }
    } else {
      serviceIdentifiers.addAll(acis!!)
    }
    serviceIdentifiers.forEach { aci ->
      try {
