...
public class SignalBaseIdentityKeyStore {
  ...
  SignalBaseIdentityKeyStore(@NonNull Context context, @NonNull IdentityTableGlue identityDatabase) {
  ...
  public @NonNull SaveResult saveIdentity(SignalProtocolAddress address, IdentityKey identityKey, boolean nonBlockingApproval) {
    try (SignalSessionLock.Lock unused = ReentrantSessionLock.INSTANCE.acquire()) {
      TI_IdentityStoreRecord identityRecord = cache.get(address.getName());
      RecipientId         recipientId    = RecipientId.fromSidOrE164(address.getName());

      if (identityRecord == null) {
        Log.i(TAG, "Saving new identity for " + address);
        cache.save(address.getName(), recipientId, identityKey, IdentityTableGlue.VerifiedStatus.DEFAULT, true, System.currentTimeMillis(), nonBlockingApproval);
      ...
      if (identityKeyChanged && Recipient.self().getId().equals(recipientId) && Objects.equals(SignalStore.account().getAci(), ServiceId.parseOrNull(address.getName()))) {
        ...
        if (identityRecord.getVerifiedStatus() != IdentityTableGlue.VerifiedStatus.DEFAULT)
        {
          verifiedStatus = IdentityTableGlue.VerifiedStatus.UNVERIFIED;
          ...
          verifiedStatus = IdentityTableGlue.VerifiedStatus.DEFAULT;
        }
  ...
  public boolean isTrustedIdentity(SignalProtocolAddress address, IdentityKey identityKey, IdentityKeyStore.Direction direction) {
    ...
    TI_IdentityStoreRecord record = cache.get(address.getName());
  ...
  public @NonNull Optional<TI_IdentityRecord> getIdentityRecord(@NonNull Recipient recipient) {
    if (recipient.hasServiceId()) {
      TI_IdentityStoreRecord record = cache.get(recipient.requireServiceId().toString());
  ...
  private boolean isTrustedForSending(@NonNull IdentityKey identityKey, @Nullable TI_IdentityStoreRecord identityRecord) {
    if (identityRecord == null) {
  ...
  private boolean isNonBlockingApprovalRequired(TI_IdentityStoreRecord record) {
  ...
  private static final class Cache {
    ...
    private final Map<String, TI_IdentityStoreRecord> cache;
    private final IdentityTableGlue                    identityDatabase;


    Cache(@NonNull IdentityTableGlue identityDatabase) {
      this.identityDatabase = identityDatabase;
      this.cache            = new LRUCache<>(200);
    }

    public @Nullable TI_IdentityStoreRecord get(@NonNull String addressName) {
      synchronized (this) {
        if (cache.containsKey(addressName)) {
          return cache.get(addressName);
        } else {
          TI_IdentityStoreRecord record = identityDatabase.getIdentityStoreRecord(addressName);
    ...
    public void save(@NonNull String addressName, @NonNull RecipientId recipientId, @NonNull IdentityKey identityKey, @NonNull IdentityTableGlue.VerifiedStatus verifiedStatus, boolean firstUse, long timestamp, boolean nonBlockingApproval) {
      withWriteLock(() -> {
        identityDatabase.saveIdentity(addressName, recipientId, identityKey, verifiedStatus, firstUse, timestamp, nonBlockingApproval);
        cache.put(addressName, new TI_IdentityStoreRecord(addressName, identityKey, verifiedStatus, firstUse, timestamp, nonBlockingApproval));
      });
    }
    ...
    public void setApproval(@NonNull String addressName, @NonNull RecipientId recipientId, @Nullable TI_IdentityStoreRecord record, boolean nonblockingApproval) {
      withWriteLock(() -> {
        identityDatabase.setApproval(addressName, recipientId, nonblockingApproval);

        if (record != null) {
          cache.put(record.getAddressName(),
                    new TI_IdentityStoreRecord(record.getAddressName(),
    ...
    public void setVerified(@NonNull String addressName, @NonNull RecipientId recipientId, @NonNull IdentityKey identityKey, @NonNull IdentityTableGlue.VerifiedStatus verifiedStatus) {
      withWriteLock(() -> {
        identityDatabase.setVerified(addressName, recipientId, identityKey, verifiedStatus);

        TI_IdentityStoreRecord record = cache.get(addressName);
        if (record != null) {
